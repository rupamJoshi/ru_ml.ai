...
NDArray::InfixExpression::InfixExpression(NDArray *left, char oper, NDArray *right)
{
	this->left = left;
	this->right = right;
	this->oper = oper;
...
{
	NDArrayTransposedView transposedView(this, is_read_only);
	transposedViewNodes.insert(&transposedView);
...
NDArray::~NDArray()
{
	...
	for (NDArrayTransposedView *tv : this->transposedViewNodes)
	{
		tv->ndArray = NULL;
	}
}
