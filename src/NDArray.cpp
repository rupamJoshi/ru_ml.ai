...
void NDArray::multiply1(InfixExpression<NDArray, NDArray> &expression)
{
	cout << "Non template multiply function" << endl;
...
std::vector<T1> &NDArray::get_raw_collection()
{
	return this->collection;
}
std::vector<uint32_t> &NDArray::get_raw_dimensions()
{
	return this->dimensions;
}
...
{
	if (expression.oper == '*')
		this->multiply(expression);
	else if (expression.oper == '/')
		this->divide(expression);
	else if (expression.oper == '+')
		this->add(expression);
	else if (expression.oper == '-')
		this->subtract(expression);
	return *this;
}
...
void NDArray::copy(NDArray &target_ndArray, vector<uint32_t> target_from, NDArray &source_ndArray, vector<uint32_t> source_from, vector<uint32_t> source_to)
{
	...
	while (true)
	{
		...
		while (i >= 0)
		{
			...
			if (source_indices[i] <= source_to[i])
				break;
...
NDArrayView NDArray::as_view(vector<uint32_t> &from, vector<uint32_t> &to)
{
	return NDArrayView(this, from, to);
}

NDArray::NDArray(const std::vector<uint32_t> &_dimensions)
{
	if (_dimensions.size() == 0)
		throw NDArrayException("Size of array not specified");
